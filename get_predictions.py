...
class prediction: 
    def __init__(self): 
        ...
        self.ids = [9939, 10002, 9941, 9999, 9930, 9935, 9994, 10004, 9943, 9942, 9993, 10005, 9936, 9937, 10001, 9933, 9991, 10003, 9990, 9988, 9986, 9985]
        ...
        for id in self.ids:
            ...
            timeseries = [{"prediction":timeseries[index]['x1']['q2'], "time":datetime.datetime.fromtimestamp(timeseries[index]['t'])}  for index in range(len(timeseries))]
            ...
            relevant_times_5 = list(filter(lambda entry: entry['time'] >= time_5, timeseries))
            diff_5 = relevant_times_5[0]['prediction'] - current_prediction
            
            relevant_times_24 = list(filter(lambda entry: entry['time'] >= time_24, timeseries))
            ...
            if abs(diff_5) > 0.05:
                if diff_5 < 0:
                    ...
                    direction = "downwards"
                self.tweets.append("The community prediction for the question\n" + "'" 
                + title + "'\n" + "on Metaculus has changed " + direction + " by more than 5% over the last 5h. \nIt is now at " + str(int(current_prediction * 100)) + "% probability.\n\n" + 
                "https://www.metaculus.com/questions/" + str(id) + "/")
            elif abs(diff_24) > 0.1:
                if diff_24 < 0:
                    direction = "upwards"
                else:
                    direction = "downwards"
                self.tweets.append("The Metaculus community prediction on\n" + "'" 
                + title + "'\n" + "has changed " + direction + " by >10% over the last 24h. \nIt's now at " + str(int(current_prediction * 100)) + "%.\n\n" + 
                "https://www.metaculus.com/questions/" + str(id) + "/")